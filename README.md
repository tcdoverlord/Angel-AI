...
## 3.3.3 Persistent Intelligence Database

This release adds a local SQLite database at `~/Angel_Platform/angel_intelligence.sqlite3` for conversations, messages, approved memories, feedback, and tool audit records. JSON history remains as a compatibility and recovery copy while migration is validated. The database is local-first and does not require a separate database server.
